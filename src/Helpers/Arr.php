...
class Arr
{
    ...
    /**
     * Remove keys from input array that do not match regex pattern
     *
     *     // Get the values "username", "password" from $_POST
     *     $auth = Arr::filterKeys($_POST, array('username', 'password'));
     * or
     ...
     * @return array
     */
    public static function filterKeysRegex(array $array, $pattern)
    {
        foreach ($array as $key => $_) {
     ...
     * @param array $array
     * @param Str|array $key
     * @param Str $otherKeys more keys as needed
     *
     * @return mixed
     */
    ...
    /**
     * $options array will be extended with $defaults values if they do not exist in $options. If a member exists in
     * $options that does not exist in $defaults, the $options member will be unset.
     *
     * @param array $options
    ...
    public static function extendOptions(array $options, array $defaults)
    {
     ...
     * @param Str $numericName name given to numeric nodes
    ...
    public static function makeHierarchy(array $array, $format, $keepKey = false)
    {
        ...
        try {
            ...
            throw new ErrorException('Invalid format pattern');
        }
        ...
        foreach ($array as $row) {
            ...
            if ($braces) {
                ...
                $cont = isset($result) ? $result : null;
            } else {
                $rows[$row[$key]] = isset($result) ? $result : null;
            }
        }
        return $rows;
    }
    ...
    /**
     * Gets column of array (can be removed if > PHP 5.5 is used - there exists array_column())
     *
     * @param array $array
     * @param string $column
     *
    ...
    /**
     * Exports array to XLSX document
     *
     * @param array $array
     * @param Str $name
     */
    //public static function toXlsx( array $array, $name = 'report' )
    //{
    //	$xlsx = new XLSXWriter();
    //	$sheetName = "Sheet1";
    //
    //	$isAssoc = Arr::isAssoc( $array );
    //	$heading = array_map( 'Text::pretifyUnderscores', array_keys( reset( $array ) ) );
    //	if ( $isAssoc ) {
    //		array_unshift( $heading, '' );
    //	}
    //	// title row
    //	$xlsx->writeSheetRow( $sheetName, $heading );
    //
    //	// $rows = array(...);
    ...
    //			array_unshift( $row, $k );
    //		}
    //		$xlsx->writeSheetRow( $sheetName, $row );
    //	}
    //
    //
